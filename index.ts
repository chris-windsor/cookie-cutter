...
async function buildDataMaps() {
    ...
    for (const [lineIdx, rawLine] of positions.split("\n").entries()) {
        ...
        if (type === 't') {
            textFields.set(key, [parseInt(page), parseFloat(x), parseFloat(y), parseInt(size)]);            
        } else if (type === 'r') {
            const currentRadioField = radioFields.get(key) || [];
            radioFields.set(key, currentRadioField.concat([[parseInt(page), parseFloat(x), parseFloat(y), parseInt(size)]]));
        } else {
            console.error(`Encounted unknown field type: '${type}' at line ${lineIdx + 1}`);
        }
...
async function modifyPdf(settings: SettingsMap) {
    ...
    for (const field of settings.radioMap.entries()) {
        ...
        const [pageIndexAbsolute, x, y, size] = radioPositions[parseInt(selectedIndex)];
